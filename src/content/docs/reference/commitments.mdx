...
```
GET /api/v1/books/commitments/:id
```
...
| `replace_lines` | boolean | no | If `true`, deletes all existing line items before adding new ones. Only applies on update/upsert when `commitment_lines_attributes` is also provided. Ignored if commitment has approved allocations. |
...
```
PUT /api/v1/books/commitments/by_external/:external_source/:external_id
```
...
```
PUT /api/v1/books/commitments/by_external/quarry/ORD-8834
```
...
By default, passing `commitment_lines_attributes` on an existing commitment **appends** new lines. To fully replace all lines, set `replace_lines: true`:

```json
{
  "commitment": {
    "expected_total_cents": 91000,
    "replace_lines": true,
    "commitment_lines_attributes": [
      { "description": "Monthly rental", "quantity": 1, "unit_price_cents": 48000 },
      { "description": "Delivery", "quantity": 1, "unit_price_cents": 21500 },
      { "description": "Removal", "quantity": 1, "unit_price_cents": 21500 }
    ]
  }
}
```

This deletes all existing lines and creates the new ones in a single operation. Only works when the commitment has no approved allocations.
...
```
PATCH /api/v1/books/commitments/:id
```

**Scope:** `write`

Same parameters as Create, including `replace_lines`. Subject to the same financial field restrictions as upsert when allocations exist.
...
```
POST /api/v1/books/commitments/:id/cancel
```
...
```
GET /api/v1/books/invoice_documents/:id/commitment_candidates
```
...
```
POST /api/v1/books/invoice_documents/:id/allocate
```
...
```
POST /api/v1/books/invoice_documents/:id/create_commitment_bill
```
...
```
POST /api/v1/books/invoice_documents
...
```
POST /api/v1/books/invoice_documents/7/allocate
{ "allocations": [{ "commitment_id": 1, "allocated_cents": 109500 }] }

POST /api/v1/books/invoice_documents/7/create_commitment_bill
{ "non_po_lines": [] }
```
...
```
POST /api/v1/books/bills/12/post_bill

POST /api/v1/books/vendor_payments
{ "vendor_payment": { "contact_id": 42, "amount_cents": 109500, "paid_at": "2026-03-15" } }

POST /api/v1/books/vendor_payments/1/apply
